...
- **Type:** `string`
- **Attributes:** Read-only  
  A globally unique identifier used to track the order publicly.
...
- **Type:** `array`
- **Attributes:** Optional  
  Total amount to be collected upon delivery (or upon return). Array of currency-amount pairs.
  
  **Supported currencies:** USD, LBP, EURO
  ...
  **Examples:**
  ...
  <Note>For RETURN orders, this amount represents what the merchant will pay on the order return operation.</Note>
